...
const getChildDatasetid: (dataset_id: string, loading?: Ref<boolean>) => Promise<Result<any>> = (
  ...
  return get(`${prefix}/${dataset_id}/child_id`, loading)
}
...
const postDataset: (data: datasetData, loading?: Ref<boolean>) => Promise<Result<any>> = (
  ...
  //return post(`${prefix}/fatherdataset`, data, undefined, loading, 1000 * 60 * 5)
}

const postfatherDataset: (data: datasetData, loading?: Ref<boolean>) => Promise<Result<any>> = (
  data,
  loading
) => {
  //return post(`${prefix}`, data, undefined, loading, 1000 * 60 * 5)
...
const getDatasetDocuments: (dataset_id: string, loading?: Ref<boolean>) => Promise<Result<any>> = (
  dataset_id,
  loading
) => {
  return get(`${prefix}/${dataset_id}/document`, loading)
}
...
const postDatasetQA: (
  ...
  return post(`${prefix}/${dataset_id}/qa_generate`, data, undefined, loading, 1000 * 60 * 5)
}
